...
if [ "$BITS" = 32 ]; then
	...
	echo
	printf "\e[1;31mPlease check the TCU self-hosting guide for alternatives.\e[0m\n"
	rm nadekopm2setup.sh
	exit 1
fi


echo -e "Welcome to TCU's pm2 setup! \nWould you like to continue? \nYour OS: $OS \nOS Version: $VER \nArchitecture: $ARCH"
...
echo
echo "TCU pm2 Installation completed..."
read -n 1 -s -p "Press any key to continue to the main menu..."
sleep 2

cd "$root"
rm "$root/TCUpm2setup.sh"
exit 0