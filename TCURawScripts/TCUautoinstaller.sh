...
if [ "$OS" = "Ubuntu" ]; then
	if [ "$VER" = "12.04" ]; then
		...
		supported=1
		VER=18.04
		echo -e "Using Ubuntu 18.04 Installation scripts.\nIf the installation fails contact Dok#4440 on Discord."
		sleep 5
	elif [ "$VER" = "19.04" ]; then
		supported=1
	elif [ "$VER" = "19.10" ]; then
...
if [ "$OS" = "Linux" ]; then
	echo -e "Your OS $OS $VER $ARCH probably can run Microsoft .NET Core. \nContact Dok#4440 on Discord with screenshot."
	rm nadekoautoinstaller.sh
	exit 1
fi

echo -e "Welcome to TCU Auto Prerequisites Installer. \nWould you like to continue? \nYour OS: $OS \nOS Version: $VER \nArchitecture: $ARCH"

while true; do
    read -p "[y/n]: " yn
    case $yn in
        [Yy]* ) clear; echo Running TCU Auto-Installer; sleep 2; break;;
        [Nn]* ) echo Quitting...; rm TCUautoinstaller.sh && exit;;
        * ) echo "Couldn't get that please type [y] for Yes or [n] for No.";;
    esac
done

if [ "$OS" = "Ubuntu" ]; then
echo "This installer will download all of the required packages for TCU. It will use about 350MB of space. This might take awhile to download if you do not have a good internet connection."
echo ""
read -n 1 -s -p "Press any key to continue..."
	if [ "$VER" = "14.04" ]; then
	echo ""
	echo "Jess was here <3"
	echo "Preparing..."
	sudo apt-get update
